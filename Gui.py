...
def update_rope(newtxt, next_window, app):
    ...
    final_r.printrope(final_r)
    next_window.destroy()
    btn = Button(app, text=text_in, command=edit_window(app))
    lst_final_content = []
    btn.pack()
...
# Delete Button
btn = Button(app, text='Save Text', command=lambda: getText(app))
btn.pack()
btn = Button(app, text='Open File', command=lambda: openfile())
btn.pack()

# : txt.delete(1.0, END))
btn = Button(app, text='Edit Text', command=lambda: edit_window(app))
btn.pack()
btn = Button(app, text='Search', command=lambda: findword())
btn.pack()
app.mainloop()
